...
public interface DisponibilityRepository extends JpaRepository<Disponibility, Long>{
    ...
    @Query(value = "SELECT distinct d.*, oc.client_id FROM disponibility d, order_client oc INNER JOIN order_client oc ON d.shirt_id = oc.shirt_id WHERE d.provider_id = :id", nativeQuery = true)
